...
$(document).ready(() => {
    ...
    $.ajax({
        ...
        success: (data) => {
            ...
            if (data.user) {
                ...
                const onlyDate = joinDate.split("T")[0]
            ...
            if (data.images && data.images.length > 0) {
                ...
                data.images.forEach((img, index) => {
                    ...
                    $item.html(
                        `<img src="${img.image_url}" class="d-block w-100 object-cover" style="height:480px;width:480px" alt="Image ${index + 1}">`,
                    )
                    $inner.append($item)
                })
            }
    ...
    $("#breadcrumbNav .breadcrumb-item a").first().on("click", function (e) {
        e.preventDefault();
    ...
    function addBreadcrumb(name, url = "#") {
        const $breadcrumb = $(`
            <li class="breadcrumb-item">
                <a class="text-decoration-none text-primary-emerald" href="${url}">${name}</a>
            </li>
        `);
    ...
    $.ajax({
        ...
        success: function (userData) {
            ...
            if (!profileUrl || profileUrl.trim() === '') {
                profileUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(userData.name || 'User')}&background=059669&color=fff&size=40&rounded=true`
            }

            const $navImg = $("#profileDropdown img")
            if ($navImg.length) {
                $navImg.attr('src', profileUrl)

                $navImg.off('error.profile').on('error.profile', function () {
                    const fallbackUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(userData.name || 'User')}&background=059669&color=fff&size=40&rounded=true`
        ...
        error: function (xhr, status, error) {
            ...
            const defaultAvatar = 'https://ui-avatars.com/api/?name=User&background=059669&color=fff&size=40&rounded=true'
            $("#profileDropdown img").attr('src', defaultAvatar)
        }
    })
}
...
function setupLogoutHandler() {
    ...
    $("#profileDropdown .dropdown-menu").off('click.logout').on('click.logout', 'a[href="#"]', function (e) {
        const $link = $(this)
        if ($link.find('i').hasClass('bi-box-arrow-right')) {
            e.preventDefault()
            logout()
        }
    })
    ...
    $("#profileDropdown .dropdown-menu").off('click.profile').on('click.profile', 'a[href="#"]', function (e) {
        const $link = $(this)
        if ($link.find('i').hasClass('bi-person')) {
            e.preventDefault()
